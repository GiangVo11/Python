...
# TODO: Read-in test portfolios (16x)
...
# TODO: Convert the date and set the index
...
pf_16       = pf_16.resample('M').ffill() # was bewirkt diese Funktion? ffill()?
pf_16.head()



# TODO: Read-in the factors for German market (RF is risk free rate):
#   1. RM (Market factor)
...
# TODO: Next, run ols regression to estimate the parameters
#
model_11   = sm.OLS(Y, X)
results_11 = model_11.fit() # Fit the model
results_11.summary()



# TODO: Redo exercise for portfolio '44'
#
...
model_44   = sm.OLS(Y, X)
results_44 = model_44.fit()    # Fit the model
results_44.summary()



# TODO: Forecasting using the CAPM
...
exp_ret_11 = results_11.params[0] * 6
exp_ret_44 = results_44.params[0] * 6

print( 'Expected return for portfolio 11: ', np.round(exp_ret_11, 2) )
print( 'Expected return for portfolio 44: ', np.round(exp_ret_44, 2) )
...
class ERP(object):
    ...
    def estimate_ERP(self, summary = True):
        ...
        #
        nr_assets = self.equity_rets.shape[1]  # E.g. 16 (portfolios)
        #print(nr_assets)

        # TODO: Create an empty vector which saves the betas for each test asset
        #
        betas = np.empty(nr_assets)
        ...
        for i in range(0, nr_assets):
            ...
            # TODO: First, calculate excess returns
            #
            pf_ex_rets     = self.equity_rets.iloc[:,i] - self.factor_rets['rf']

            market_ex_rets = self.factor_rets['rm'] - self.factor_rets['rf']
        ...
        #  Hint: use the '.shape()' function
        #
        T          =  self.equity_rets.shape[0]   # Number of asset returns (time series)


        # TODO: Create an empty vector which saves the lambdas and alphas for each time point
        #
        mrp_all    = np.empty(T)
        alphas_all = np.empty(T)
        ...
        for i in range(0, T):
            ...
            # TODO: First, calculate excess returns
            #
            pf_ex_rets    = self.equity_rets.iloc[i,:] - self.factor_rets.iloc[i, 1] # HIER FEHLT NOCH RICHTIG VIEL!
            ...
            mrp_all[i]    = results.params[1]
            alphas_all[i] = results.params[0]


        # TODO: Calculate average alpha and lambda
        #
        alpha  = np.mean(alphas_all)
        mrp    = np.mean(mrp_all)
        ...
        t_stat_mrp = mrp / ( np.std(mrp_all) / np.sqrt(T) )
...
# TODO: Estimate ERP
#
erp_ger       =   ERP(pf_16, factors)  # Instantiate the object with the previously created data frames for the test assets and factors
erp_ger_est   =   erp_ger.estimate_ERP()  # Run the .estimate_ERP function
...
#
X            = pd.DataFrame(factors['rm']-factors['rf'])
X.columns    = ['rm']
...
#
model_11   = sm.OLS(Y, X)
results_11 = model_11.fit() # Fit model
...
model_44   = sm.OLS(Y, X)
results_44 = model_44.fit() # Fit model
results_44.summary()

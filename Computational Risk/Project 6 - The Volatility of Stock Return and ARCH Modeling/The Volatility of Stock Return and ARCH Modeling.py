...
#
es50_id                 = pd.read_csv('es50_id.csv', sep = ';')
es50_id.head()


# TODO: Format date and set as index
#
es50_id['loctimestamp'] = pd.to_datetime(es50_id['loctimestamp'])               # Format the date
...
#
date = '2010-06-10'
es50_id_sub = es50_id.ix['2010-06-10']    # Select index values of specific day
es50_id_sub.head()
...
#
es50_id_sub['returns'].plot(figsize=(15,6))



# TODO: Calculate cumulative returns
#
es50_id_sub['cum_returns'] = es50_id_sub['returns'].cumsum()
...
#
es50_id_sub['cum_returns'].plot(figsize=(15,6))
...
es50_id_sub['cum_returns'] = es50_id_sub['cum_returns'].astype(float)

es50_id_sub['cum_returns'].plot( color = 'red' )
...
#
es50_id_sub['volatility'] = pd.rolling_sum(es50_id_sub['returns_squ'], window = 20)
es50_id_sub[15:25]
...
#
es50_id_sub['returns'].plot(figsize=(15,6))
es50_id_sub['volatility'].plot(figsize=(15,6))
...
es50_id_sub['volatility'].loc[start:end].plot( color = 'navy', figsize=(15,6))
plt.title( 'ECB Monetary Policy Decision: ' + date)
...
class ARMA_ARCH(object):
    ...
    def two_pass_est( self, print_results = False, plot_results = False ):
        ...
        #
        model_vol            = ZeroMean(resid)         # Instantiate object (mean equation)
        model_vol.volatility = ARCH(p=1)         # Specify variance equation
        results_vol          = model_vol.fit()        # Fit the model

        volatility           = results_vol.conditional_volatility      # NO IDEA WHAT TO DO HERE # Extract the volatility
...
arch_vol, est_results_vol, resid_squ  = model_arch.two_pass_est(print_results = True, plot_results = True )        # Run two pass estimation
...
es50_id_sub[['volatility_arch_2pass', 'volatility_arch_package']].plot( subplots = False, figsize=(15,6) )

es50_id_sub[['volatility_arch_2pass', 'volatility_arch_package', 'volatility']].plot( subplots = False, figsize=(15,6) )


# TODO: Obtain correlation matrix between volatility proxies
#
es50_id_sub[['volatility_arch_2pass', 'volatility_arch_package', 'volatility']].corr()
...
alpha_1_hat = alpha_1_hat


# TODO: Forecast 1, 2 and 3 period(s) ahead
exp_var_h1 = alpha_0_hat + alpha_1_hat * last_value
exp_var_h2 = alpha_0_hat + alpha_1_hat * exp_var_h1
exp_var_h3 = alpha_0_hat + alpha_1_hat * exp_var_h2
